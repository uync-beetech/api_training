    ...
    public void updateView(Product product) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            try {
                ...
                if (optionalLastProductView.isPresent()) {
                    // Increment the view count by one
                    viewCount = optionalLastProductView.get().getViewCount();
                }
                viewCount++;
                productViewRepository.createProductView(product.getId(), viewCount, stringToday);
